...
public class AppCompatSpinner extends Spinner implements TintableBackgroundView {
    ...
    private class DropdownPopup extends ListPopupWindow {
        ...
        class C02002 implements OnGlobalLayoutListener {
            C02002() {
            }
        ...
        public void show() {
            ...
            if (!wasShowing) {
                ...
                if (vto != null) {
                    final OnGlobalLayoutListener layoutListener = new C02002();
    ...
    public AppCompatSpinner(Context context, int mode) {
        this(context, null, C0180R.attr.spinnerStyle, mode);
    }

    public AppCompatSpinner(Context context, AttributeSet attrs) {
        this(context, attrs, C0180R.attr.spinnerStyle);
    }
    ...
    public AppCompatSpinner(Context context, AttributeSet attrs, int defStyleAttr, int mode, Theme popupTheme) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0180R.styleable.Spinner, defStyleAttr, 0);
        this.mBackgroundTintHelper = new AppCompatBackgroundHelper(this);
        if (popupTheme != null) {
            this.mPopupContext = new ContextThemeWrapper(context, popupTheme);
        } else {
            int popupThemeResId = a.getResourceId(C0180R.styleable.Spinner_popupTheme, 0);
        ...
        if (this.mPopupContext != null) {
            ...
            if (mode == 1) {
                ...
                TintTypedArray pa = TintTypedArray.obtainStyledAttributes(this.mPopupContext, attrs, C0180R.styleable.Spinner, defStyleAttr, 0);
                this.mDropDownWidth = pa.getLayoutDimension(C0180R.styleable.Spinner_android_dropDownWidth, -2);
                popup.setBackgroundDrawable(pa.getDrawable(C0180R.styleable.Spinner_android_popupBackground));
                popup.setPromptText(a.getString(C0180R.styleable.Spinner_android_prompt));
                pa.recycle();
                this.mPopup = popup;
        ...
        CharSequence[] entries = a.getTextArray(C0180R.styleable.Spinner_android_entries);
        if (entries != null) {
            SpinnerAdapter adapter = new ArrayAdapter(context, 17367048, entries);
            adapter.setDropDownViewResource(C0180R.layout.support_simple_spinner_dropdown_item);
            setAdapter(adapter);
        }
        a.recycle();
        this.mPopupSet = true;

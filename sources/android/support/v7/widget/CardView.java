...
import android.support.v7.cardview.C0181R;
...
public class CardView extends FrameLayout {
    ...
    private final CardViewDelegate mCardViewDelegate = new C04821();
    ...
    class C04821 implements CardViewDelegate {
    ...
    private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, C0181R.styleable.CardView, defStyleAttr, C0181R.style.CardView);
        if (a.hasValue(C0181R.styleable.CardView_cardBackgroundColor)) {
            backgroundColor = a.getColorStateList(C0181R.styleable.CardView_cardBackgroundColor);
        } else {
            int color;
            ...
            if (hsv[2] > 0.5f) {
                color = getResources().getColor(C0181R.color.cardview_light_background);
            } else {
                color = getResources().getColor(C0181R.color.cardview_dark_background);
            }
            backgroundColor = ColorStateList.valueOf(color);
        }
        float radius = a.getDimension(C0181R.styleable.CardView_cardCornerRadius, 0.0f);
        float elevation = a.getDimension(C0181R.styleable.CardView_cardElevation, 0.0f);
        float maxElevation = a.getDimension(C0181R.styleable.CardView_cardMaxElevation, 0.0f);
        this.mCompatPadding = a.getBoolean(C0181R.styleable.CardView_cardUseCompatPadding, false);
        this.mPreventCornerOverlap = a.getBoolean(C0181R.styleable.CardView_cardPreventCornerOverlap, true);
        int defaultPadding = a.getDimensionPixelSize(C0181R.styleable.CardView_contentPadding, 0);
        this.mContentPadding.left = a.getDimensionPixelSize(C0181R.styleable.CardView_contentPaddingLeft, defaultPadding);
        this.mContentPadding.top = a.getDimensionPixelSize(C0181R.styleable.CardView_contentPaddingTop, defaultPadding);
        this.mContentPadding.right = a.getDimensionPixelSize(C0181R.styleable.CardView_contentPaddingRight, defaultPadding);
        this.mContentPadding.bottom = a.getDimensionPixelSize(C0181R.styleable.CardView_contentPaddingBottom, defaultPadding);
        if (elevation > maxElevation) {
            maxElevation = elevation;
        }
        this.mUserSetMinWidth = a.getDimensionPixelSize(C0181R.styleable.CardView_android_minWidth, 0);
        this.mUserSetMinHeight = a.getDimensionPixelSize(C0181R.styleable.CardView_android_minHeight, 0);
        a.recycle();

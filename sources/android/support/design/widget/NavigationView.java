...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    class C04131 implements Callback {
        C04131() {
        }
    ...
    public static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C04141());
        public Bundle menuState;

        /* renamed from: android.support.design.widget.NavigationView$SavedState$1 */
        static class C04141 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C04141() {
            }
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0010R.styleable.NavigationView, defStyleAttr, C0010R.style.Widget_Design_NavigationView);
        ViewCompat.setBackground(this, a.getDrawable(C0010R.styleable.NavigationView_android_background));
        if (a.hasValue(C0010R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0010R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a.getBoolean(C0010R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = a.getDimensionPixelSize(C0010R.styleable.NavigationView_android_maxWidth, 0);
        if (a.hasValue(C0010R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a.getColorStateList(C0010R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a.hasValue(C0010R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a.getResourceId(C0010R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = null;
        if (a.hasValue(C0010R.styleable.NavigationView_itemTextColor)) {
            itemTextColor = a.getColorStateList(C0010R.styleable.NavigationView_itemTextColor);
        }
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a.getDrawable(C0010R.styleable.NavigationView_itemBackground);
        this.mMenu.setCallback(new C04131());
        ...
        if (a.hasValue(C0010R.styleable.NavigationView_menu)) {
            inflateMenu(a.getResourceId(C0010R.styleable.NavigationView_menu, 0));
        }
        if (a.hasValue(C0010R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a.getResourceId(C0010R.styleable.NavigationView_headerLayout, 0));
        }
        a.recycle();
    }
    ...
    private ColorStateList createDefaultColorStateList(int baseColorThemeAttr) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0180R.attr.colorPrimary, value, true)) {
            return null;
        }

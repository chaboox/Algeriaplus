...
public class ImageRequest extends Request<Bitmap> {
    ...
    protected Response<Bitmap> parseNetworkResponse(NetworkResponse response) {
        ...
        synchronized (sDecodeLock) {
            try {
                ...
                VolleyLog.m10e("Caught OOM for %d byte image, url=%s", Integer.valueOf(response.data.length), getUrl());
    ...
    static int findBestSampleSize(int actualWidth, int actualHeight, int desiredWidth, int desiredHeight) {
        float n = 1.0f;
        while (((double) (IMAGE_BACKOFF_MULT * n)) <= Math.min(((double) actualWidth) / ((double) desiredWidth), ((double) actualHeight) / ((double) desiredHeight))) {
